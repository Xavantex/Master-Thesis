...
A function which was found to be unecessary was an \texttt{if} condition checking that all elements in an array were not empty, this check took more than ten percent of the execution time for \texttt{mvpa\_datapartition}.
...
After replacing the \texttt{if} statement with \texttt{try/except} keywords, the execution time compared to before when profiling was ten percent less.
After testing no difference in results were noticed as well.%Write about it being same results as well.

Due to the highly nested and long for loops in the original code, small performance gains were found in using the built in map function in Python, which could save a few lines of code.
