...
%Rule 3, plain functions are called faster than class methods.
...
%Rule 5, Avoid using NumPy functions with scalar arguments.

%Rule 6, use xrange instead of range.
...
%Rule 8, avoid resizing NumPy arrays.
%Rule 9, let eval and exec work on compiled code.
%Rule 10, callbacks to Python from fortran and C/C++ are expensive.
%Rule 11, avoid unecessary allocation of large data structures.
%Rule 12, be careful with type matching in mixed Python-F77 software.
%Rule 13, F2PY may introduce time-consuming copying of arrays.
%Rule 14, Calling C++ classes via SWIG involves proxy classes.
%Rule 15, wrap2callable may be expensive.
...
Python's scikit-learn package does allow someone to write their own error function, however MATLAB's documentation is not clear on exactly how it's default settings actually calculate their error. %This might be more of constraints issue, from BoxConstraint Try and elaborate on the ERROR, find sources
...
%Communication overhead? Could be very real since it's probably too divided now
